...
namespace Imagine.WebAR
{
    ...
    public partial class WorldTracker : MonoBehaviour
    {
        ...
        private Vector3 origPos, origScale = Vector3.zero;
        private Quaternion origRot = Quaternion.identity;
        ...
        public void PlaceOrigin()
        {
            ...
            if (usePlacementIndicator)
            {
                ...
                if (ps.placed)
                {
                    Debug.LogWarning("Origin is already placed. Call ResetOrigin() first");
                    return;
                }
            ...
            WebARManager.Instance.StartCoroutine(WaitAndInvoke(0.25f, ()=>{
        ...
        public void ResetOrigin()
        {
            if (trackerCamera == null)
            {
                Debug.LogWarning("Tracker Camera not set. Set it in the inspector");
                return;
            }
            ...
            if (usePlacementIndicator)
            {
                ...
                if (!ps.placed)
                {
                    Debug.LogWarning("Origin not placed. Call PlaceOrigin() first");
                    return;
                }
            ...
            foreach(var g in eventSettings.ShowGameObjectsWhenPlaced){
                if(g == null)
                {
                    Debug.LogWarning("A null object ignored in ShowGameObjectsWhenPlaced list");
                    continue;
                }
                g.gameObject.SetActive(false);
            }
            foreach(var g in eventSettings.ShowGameObjectsWhenReset){
                if(g == null)
                {
                    Debug.LogWarning("A null object ignored in ShowGameObjectsWhenReset list");
                    continue;
                }
                //Debug.Log(g.name);
                g.gameObject.SetActive(true);
